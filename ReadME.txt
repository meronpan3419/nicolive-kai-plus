...
■更新履歴
#2012/05/03 nicolive kai p11 イマココの高度情報をEGM96でジオイド補正（適当版　@ALTITUDE＿EGM96 http://earth-info.nga.mil/GandG/wgs84/gravitymod/egm96/binary/WW15MGH.DAC をnicolive.exeと同じフォルダにポイッ）。
。次枠取り（F２）で出たダイヤログをF2でOKボタン押せるように。デバッグコード追加。放送接続エラーを棒読みで通知に変更してダイアログが出ないように。自動最接続時にクッキーの有効チェックを行わないように（電波の悪い所ではページ読み込み多発で死ぬ）。配信コンソール読み込みfix。win vista/7のIE保護モードクッキー問題fix。コネクションチェック強化バグfix。枠取れるのに枠取りした時に出る予期せぬエラーバグfix。
